    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-6 p-8">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">{t('terminal:terminalGrid.agentTerminals')}</h2>
            <p className="mt-1 text-sm text-muted-foreground max-w-md">
              {t('terminal:terminalGrid.agentTerminalsDescription', {
        ...
        <Button onClick={handleAddTerminal} className="gap-2">
          ...
          {t('terminal:terminalGrid.newTerminal')}
        </Button>
      </div>
    );
  }
        ...
        <div className="flex h-10 items-center justify-between border-b border-border bg-card/30 px-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-muted-foreground">
              {t('terminal:terminalGrid.terminalCount', { count: terminals.length })}
            </span>
          </div>
          <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    ...
                    {t('terminal:terminalGrid.history')}
                    <ChevronDown className="h-3 w-3" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  <div className="px-2 py-1.5 text-xs font-medium text-muted-foreground">
                    {t('terminal:terminalGrid.restoreSessionsFrom')}
                  </div>
                  <DropdownMenuSeparator />
                  {sessionDates.map((dateInfo) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t('terminal:terminalGrid.sessionCount', { count: dateInfo.sessionCount })}
                      </span>
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
            ...
            {terminals.some((t) => t.status === 'running' && !t.isCLIMode) && (
              ...
              >
                ...
                {t('terminal:terminalGrid.invokeClaudeAll')}
              </Button>
            )}
            <Button
              variant="outline"
