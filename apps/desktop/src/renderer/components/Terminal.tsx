...
import { LineInsertContextMenu } from './terminal/LineInsertContextMenu';
...
export const Terminal = forwardRef<TerminalHandle, TerminalProps>(function Terminal({
  ...
  // Context menu for line insertion
  const { contextMenu, handleContextMenu, closeContextMenu, menuRef } =
  ...
  // ---- Line Insert Handlers (Context Menu Actions) ----

  /**
   * Insert a blank line ABOVE the current cursor position.
   * Uses VT escape sequences:
   * - \x1b[{n}A  - move cursor up n lines
   * - \x1b[{n}L  - insert n lines above cursor
   * Then restore cursor position.
   */
  const handleInsertLineAbove = useCallback(() => {
    closeContextMenu();
    ...
    if (cursorY === 0) {
      // Already at top of viewport, just send newline to start new line
      window.electronAPI.sendTerminalInput(id, '\r');
      return;
    }

    // Move cursor up one line, insert a blank line there
    // This effectively creates a blank line above current line
    const up = '\x1b[1A';     // Move cursor up 1
    const insert = '\x1b[1L';  // Insert 1 line above cursor
    const down = '\x1b[1B';   // Move cursor back down

    window.electronAPI.sendTerminalInput(id, up + insert + down);
  }, [id, closeContextMenu]);

  /**
   * Insert a blank line BELOW the current cursor position.
   * Uses VT escape sequence \x1b[{n}L - insert n lines below cursor.
   * This pushes current line and all below it down by one row.
   */
  const handleInsertLineBelow = useCallback(() => {
    closeContextMenu();
    if (!isCreatedRef.current) return;

    // Insert 1 line below current cursor position
    window.electronAPI.sendTerminalInput(id, '\x1b[1L');
  }, [id, closeContextMenu]);
...
Please confirm you're ready by saying: I'm ready to work on ${selectedTask.title} - Context is loaded.`;
  ...
  const onTerminalContextMenu = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    const terminal = xtermRef.current;
    if (!terminal) return;

    // Calculate which row was clicked based on terminal cursor + click position
    // Use cursorY as approximate row, offset by click position relative to terminal height
    const termDiv = terminalRef.current;
    if (!termDiv) return;

    const termHeight = termDiv.clientHeight;
    const clickY = e.nativeEvent.offsetY;
    const rowHeight = termHeight / rows;
    const cursorRow = Math.floor(clickY / rowHeight) + terminal.buffer.active.cursorY;

    handleContextMenu(e, cursorRow);
  }, [xtermRef, terminalRef, rows, handleContextMenu]);
  ...
  return (
    ...
    >
      ...
      {/* Line Insert Context Menu */}
      {contextMenu && (
        <LineInsertContextMenu
          contextMenu={contextMenu}
          menuRef={menuRef}
          onInsertAbove={handleInsertLineAbove}
